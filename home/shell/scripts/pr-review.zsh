# pr-review コマンドの本体。
#
# default.nix が shebang と `pr-review "$@"` を足して PATH 上の実行ファイルに
# 仕立てる。zsh 関数ではなく独立したプロセスにしてあるのは、nushell や fish
# からも呼べるようにするため (シェル関数は子プロセスに継承されない)。
#
# このファイル自体は定義だけを行うので、テストから source してヘルパーだけを
# 読み込むこともできる。
#
# 中身が zsh 固有 (emulate / setopt err_return / ${match} / ${(q)} / ${x:h}) な
# ので、shebang は zsh でなければならない。
